...
void CMain::JSONUpdateThread(void *pUser)
{
	...
	while(gs_Running)
	{
		...
		if(!m_pJSONUpdateThreadData->m_ReloadRequired)
			str_format(pBuf - 2, BufSize - (pBuf - aFileBuf), "\n],\n\"updated\": \"%lld\"\n}", (long long)time(/*ago*/0));
		else
		{
			str_format(pBuf - 2, BufSize - (pBuf - aFileBuf), "\n],\n\"updated\": \"%lld\",\n\"reload\": true\n}", (long long)time(/*ago*/0));
...
int CMain::ReadConfigWebUsers()
{
	...
	m_HttpServer.ClearUsers();
	const json_value &rWeb = (*pJsonData)["web_users"];
	if(rWeb.type == json_array)
	{
		for(unsigned i = 0; i < rWeb.u.array.length; i++)
		{
			const char *pUser = rWeb[i]["username"].u.string.ptr;
			const char *pPass = rWeb[i]["password"].u.string.ptr;
			if(pUser && pPass)
				m_HttpServer.AddUser(pUser, pPass);
		}
	}
...
int CMain::Run()
{
	...
	// Start HTTP server
	if(!m_HttpServer.Init(m_Config.m_aWebDir, m_Config.m_HttpPort))
		dbg_msg("main", "Warning: HTTP server failed to start on port %d", m_Config.m_HttpPort);
...
int main(int argc, const char *argv[])
{
	...
	struct argparse_option aOptions[] = {
		...
		OPT_INTEGER(0, "http-port", &Config.m_HttpPort, "HTTP server port for web dashboard", 0),
		OPT_END(),
	};
