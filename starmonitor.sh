...
file="/usr/local/StarMonitor"
web_file="/usr/local/StarMonitor/web"
server_file="/usr/local/StarMonitor/server"
server_conf="/usr/local/StarMonitor/server/config.json"
server_conf_1="/usr/local/StarMonitor/server/config.conf"
client_file="/usr/local/StarMonitor/client"

client_log_file="/tmp/starmonitor_client.log"
server_log_file="/tmp/starmonitor_server.log"
jq_file="${file}/jq"
...
check_sys() {
  if [[ -f /etc/redhat-release ]]; then
    ...
    echo -e "StarMonitor 暂不支持该Linux发行版"
  fi
  bit=$(uname -m)
}
check_installed_server_status() {
  [[ ! -e "${server_file}/sergate" ]] && echo -e "${Error} StarMonitor 服务端没有安装，请检查 !" && exit 1
}
check_installed_client_status() {
  [[ ! -e "${client_file}/status-client.py" ]] && echo -e "${Error} StarMonitor 客户端没有安装，请检查 !" && exit 1
}
check_pid_server() {
  PID=$(pgrep -f "sergate")
}
...
Service_StarMonitor_server() {
  Download_StarMonitor_Service "server"
}
Service_StarMonitor_client() {
  Download_StarMonitor_Service "client"
}
...
"web_users":
 [
  {
   "username": "${web_username_s}",
   "password": "${web_password_s}"
  }
 ]
}
EOF
...
Read_config_server() {
  if [[ ! -e "${server_conf_1}" ]]; then
    ...
    http_port="$(grep "HTTP_PORT = " ${server_conf_1} | awk '{print $3}')"
...
Set_server() {
  ...
  [[ -z ${mode} ]] && mode="server"
  if [[ ${mode} == "server" ]]; then
    echo -e "请输入 StarMonitor 服务端中网站要设置的 域名[server]
默认为本机IP为域名，例如输入: toyoo.pw ，如果要使用本机IP，请留空直接回车"
    read -erp "(默认: 本机IP):" server_s
    [[ -z "$server_s" ]] && server_s=""
  else
    echo -e "请输入 StarMonitor 服务端的 IP/域名[server]，请注意，如果你的域名使用了CDN，请直接填写IP"
...
Set_server_http_port() {
  while true; do
    echo -e "请输入 StarMonitor 服务端中网站要设置的 域名/IP的端口[1-65535]（如果是域名的话，一般用 80 端口）"
...
Set_http_port() {
  while true; do
    echo -e "请输入 StarMonitor 内嵌仪表盘 HTTP 端口[1-65535]（用于 Web 登录查看监控数据）"
    read -erp "(默认: 8080):" http_port_s
    [[ -z "$http_port_s" ]] && http_port_s="8080"
    if [[ "$http_port_s" =~ ^[0-9]*$ ]]; then
      if [[ ${http_port_s} -ge 1 ]] && [[ ${http_port_s} -le 65535 ]]; then
        echo && echo "	================================================"
        echo -e "	仪表盘端口: ${Red_background_prefix} ${http_port_s} ${Font_color_suffix}"
        echo "	================================================" && echo
        break
      else
        echo "输入错误, 请输入正确的端口。"
...
Set_web_username() {
  echo -e "请输入 Web 登录用户名（用于浏览器访问仪表盘）"
  read -erp "(默认: admin):" web_username_s
  [[ -z "$web_username_s" ]] && web_username_s="admin"
  echo && echo "	================================================"
  echo -e "	Web 用户名: ${Red_background_prefix} ${web_username_s} ${Font_color_suffix}"
  echo "	================================================" && echo
}
Set_web_password() {
  echo -e "请输入 Web 登录密码"
  read -erp "(默认: admin123):" web_password_s
  [[ -z "$web_password_s" ]] && web_password_s="admin123"
  echo && echo "	================================================"
  echo -e "	Web 密码: ${Red_background_prefix} ${web_password_s} ${Font_color_suffix}"
  echo "	================================================" && echo
}
Set_server_port() {
  while true; do
    echo -e "请输入 StarMonitor 服务端监听的端口[1-65535]（用于服务端接收客户端消息的端口，客户端要填写这个端口）"
...
Set_username() {
  ...
  [[ -z ${mode} ]] && mode="server"
  if [[ ${mode} == "server" ]]; then
    echo -e "请输入 StarMonitor 服务端要设置的用户名[username]（字母/数字，不可与其他账号重复）"
  else
    echo -e "请输入 StarMonitor 服务端中对应配置的用户名[username]（字母/数字，不可与其他账号重复）"
  fi
...
Set_password() {
  ...
  [[ -z ${mode} ]] && mode="server"
  if [[ ${mode} == "server" ]]; then
    echo -e "请输入 StarMonitor 服务端要设置的密码[password]（字母/数字，可重复）"
  else
    echo -e "请输入 StarMonitor 服务端中对应配置的密码[password]（字母/数字）"
  fi
...
Set_name() {
  echo -e "请输入 StarMonitor 服务端要设置的节点名称[name]（支持中文，前提是你的系统和SSH工具支持中文输入，仅仅是个名字）"
...
Set_type() {
  echo -e "请输入 StarMonitor 服务端要设置的节点虚拟化类型[type]（例如 OpenVZ / KVM）"
...
Set_location() {
  echo -e "请输入 StarMonitor 服务端要设置的节点位置[location]（支持中文，前提是你的系统和SSH工具支持中文输入）"
...
Set_region() {
  echo -e "请输入 StarMonitor 服务端要设置的节点地区[region]（用于国家/地区的旗帜图标显示）"
...
————————
 ${Green_font_prefix} 11.${Font_color_suffix} 修改 服务端监听端口
 ${Green_font_prefix} 12.${Font_color_suffix} 修改 仪表盘 HTTP 端口
 ${Green_font_prefix} 13.${Font_color_suffix} 修改 Web 登录用户名/密码" && echo
  read -erp "(默认: 取消):" server_num
  [[ -z "${server_num}" ]] && echo "已取消..." && exit 1
  if [[ ${server_num} == "1" ]]; then
    Add_StarMonitor_server
  elif [[ ${server_num} == "2" ]]; then
    Del_StarMonitor_server
  elif [[ ${server_num} == "3" ]]; then
    Modify_StarMonitor_server_username
  elif [[ ${server_num} == "4" ]]; then
    Modify_StarMonitor_server_password
  elif [[ ${server_num} == "5" ]]; then
    Modify_StarMonitor_server_name
  elif [[ ${server_num} == "6" ]]; then
    Modify_StarMonitor_server_type
  elif [[ ${server_num} == "7" ]]; then
    Modify_StarMonitor_server_location
  elif [[ ${server_num} == "8" ]]; then
    Modify_StarMonitor_server_region
  elif [[ ${server_num} == "9" ]]; then
    Modify_StarMonitor_server_all
  elif [[ ${server_num} == "10" ]]; then
    Modify_StarMonitor_server_disabled
    ...
    Set_http_port
    Write_server_config_conf
  elif [[ ${server_num} == "13" ]]; then
    Set_web_username
    Set_web_password
    ${jq_file} '.web_users = [{"username": "'"${web_username_s}"'", "password": "'"${web_password_s}"'"}]' ${server_conf} > ${server_conf}.tmp && mv ${server_conf}.tmp ${server_conf}
    echo -e "${Info} Web 登录信息已更新"
  else
    echo -e "${Error} 请输入正确的数字[1-13]" && exit 1
  fi
  Restart_StarMonitor_server
}
List_StarMonitor_server() {
...
Set_StarMonitor_client() {
  check_installed_client_status
  Set_config_client
  Read_config_client
  Modify_config_client
  Restart_StarMonitor_client
}
Install_vnStat() {
  if [[ ${release} == "archlinux" ]]; then
...
Install_caddy() {
  ...
  ${Green_font_prefix} 2.${Font_color_suffix} HTTPS（需要域名，自动申请 Let's Encrypt 证书）"
    read -erp "请输入数字 [1-2], 默认为 1:" caddy_mode
    [[ -z "${caddy_mode}" ]] && caddy_mode=1
    if [[ ${caddy_mode} == "2" ]]; then
      echo -e "请输入你的域名（需已解析到本服务器IP）："
      read -erp "(例如: status.example.com):" server_domain
      [[ -z "$server_domain" ]] && echo "已取消..." && exit 1
      echo && echo "	================================================"
      echo -e "	域名: ${Red_background_prefix} ${server_domain} ${Font_color_suffix}"
      echo "	================================================" && echo
      cat >${caddy_file} <<-EOF
${server_domain} {
...
EOF
    ...
    echo -e "${Info} 跳过 HTTP服务部署，请手动部署，Web网页文件位置：${web_file} ，如果位置改变，请注意修改服务脚本文件 /etc/init.d/starmonitor-server 中的 WEB_BIN 变量 !"
  fi
}
...
Update_StarMonitor_server() {
  ...
  if [[ -n ${PID} ]]; then
    if [[ ${release} == "archlinux" ]]; then
      systemctl stop starmonitor-server
    else
      /etc/init.d/starmonitor-server stop
    fi
  fi
  Download_StarMonitor_server
  rm -rf /etc/init.d/starmonitor-server
  Service_StarMonitor_server
  Start_StarMonitor_server
}
Update_StarMonitor_client() {
  ...
  if [[ -n ${PID} ]]; then
    if [[ ${release} == "archlinux" ]]; then
      systemctl stop starmonitor-client
    else
      /etc/init.d/starmonitor-client stop
    fi
  fi
  if [[ ! -e "${client_file}/status-client.py" ]]; then
    if [[ ! -e "${file}/status-client.py" ]]; then
      echo -e "${Error} StarMonitor 客户端文件不存在 !" && exit 1
    else
  ...
  Read_config_client
  Modify_config_client
  rm -rf /etc/init.d/starmonitor-client
  Service_StarMonitor_client
  Start_StarMonitor_client
}
Start_StarMonitor_server() {
  check_installed_server_status
  check_pid_server
  [[ -n ${PID} ]] && echo -e "${Error} StarMonitor 正在运行，请检查 !" && exit 1
  if [[ ${release} == "archlinux" ]]; then
    systemctl start starmonitor-server.service
  else
    /etc/init.d/starmonitor-server start
  fi
}
Stop_StarMonitor_server() {
  check_installed_server_status
  check_pid_server
  [[ -z ${PID} ]] && echo -e "${Error} StarMonitor 没有运行，请检查 !" && exit 1
  if [[ ${release} == "archlinux" ]]; then
    systemctl stop starmonitor-server.service
  else
    /etc/init.d/starmonitor-server stop
  fi
}
...
Uninstall_StarMonitor_server() {
  check_installed_server_status
  echo "确定要卸载 StarMonitor 服务端(如果同时安装了客户端，则只会删除服务端) ? [y/N]"
  echo
  read -erp "(默认: n):" unyn
  [[ -z ${unyn} ]] && unyn="n"
  if [[ ${unyn} == [Yy] ]]; then
    ...
    if [[ ${release} == "centos" ]]; then
      chkconfig --del starmonitor-server
    elif [[ ${release} == "debian" ]]; then
      update-rc.d -f starmonitor-server remove
    elif [[ ${release} == "archlinux" ]]; then
      systemctl stop starmonitor-server
      systemctl disable starmonitor-server
      rm /usr/lib/systemd/system/starmonitor-server.service
    fi
    echo && echo "StarMonitor 卸载完成 !" && echo
  else
...
Start_StarMonitor_client() {
  ...
  [[ -n ${PID} ]] && echo -e "${Error} StarMonitor 正在运行，请检查 !" && exit 1
  if [[ ${release} == "archlinux" ]]; then
    systemctl start starmonitor-client.service
  else
    /etc/init.d/starmonitor-client start
  fi
}
Stop_StarMonitor_client() {
  check_installed_client_status
  check_pid_client
  [[ -z ${PID} ]] && echo -e "${Error} StarMonitor 没有运行，请检查 !" && exit 1
  if [[ ${release} == "archlinux" ]]; then
    systemctl stop starmonitor-client.service
  else
    /etc/init.d/starmonitor-client stop
  fi
}
Restart_StarMonitor_client() {
  ...
  if [[ -n ${PID} ]]; then
    if [[ ${release} == "archlinux" ]]; then
      systemctl restart starmonitor-client.service
    else
      /etc/init.d/starmonitor-client restart
    fi
  fi
}
Uninstall_StarMonitor_client() {
  check_installed_client_status
  echo "确定要卸载 StarMonitor 客户端(如果同时安装了服务端，则只会删除客户端) ? [y/N]"
  echo
  read -erp "(默认: n):" unyn
  [[ -z ${unyn} ]] && unyn="n"
  if [[ ${unyn} == [Yy] ]]; then
    ...
    rm -rf /etc/init.d/starmonitor-client
    if [[ ${release} == "centos" ]]; then
      chkconfig --del starmonitor-client
    elif [[ ${release} == "debian" ]]; then
      update-rc.d -f starmonitor-client remove
    elif [[ ${release} == "archlinux" ]]; then
      systemctl stop starmonitor-client
      systemctl disable starmonitor-client
      rm /usr/lib/systemd/system/starmonitor-client.service
    fi
    echo && echo "StarMonitor 卸载完成 !" && echo
  else
    echo && echo "卸载已取消..." && echo
...
View_StarMonitor_client() {
  check_installed_client_status
  Read_config_client
  clear && echo "————————————————————" && echo
  echo -e "  StarMonitor 客户端配置信息：
...
Update_Shell() {
  if [[ -e "/etc/init.d/starmonitor-client" ]] || [[ -e "/usr/lib/systemd/system/starmonitor-client.service" ]]; then
    rm -rf /etc/init.d/starmonitor-client
    rm -rf /usr/lib/systemd/system/starmonitor-client.service
    Service_StarMonitor_client
  fi
  if [[ -e "/etc/init.d/starmonitor-server" ]] || [[ -e "/usr/lib/systemd/system/starmonitor-server.service" ]]; then
    rm -rf /etc/init.d/starmonitor-server
    rm -rf /usr/lib/systemd/system/starmonitor-server.service
    Service_StarMonitor_server
  fi
  echo -e "脚本已更新 !" && exit 0
}
